...
const konfigurerAzure = () => {
    ...
    switch (process.env.ENV) {
        ...
        case 'local':
            process.env.AAD_LOGOUT_REDIRECT_URL = `https://login.microsoftonline.com/navq.onmicrosoft.com/oauth2/logout?post_logout_redirect_uri=http:\\\\localhost:8020`;
            process.env.AAD_REDIRECT_URL = 'http://localhost:8020/auth/openid/callback';
            ...
            break;
        case 'e2e':
            process.env.AAD_LOGOUT_REDIRECT_URL = `http://host.docker.internal:1111/v2.0/logout?post_logout_redirect_uri=http:\\\\localhost:8020`;
            process.env.AAD_REDIRECT_URL = 'http://host.docker.internal:8020/auth/openid/callback';
